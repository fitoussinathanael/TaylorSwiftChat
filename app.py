...
mode = st.sidebar.radio(
    "Navigation",
    ["🏠 Dashboard", "👤 Client", "⚖️ Avocat", "🤖 Agent IA", "🗂️ Archives", "👥 Clients"]  # AJOUT
)

st.sidebar.markdown("---")

# SHADOW MODE GLOBAL
st.session_state.shadow_mode = st.sidebar.toggle("👻 Shadow Mode", value=st.session_state.shadow_mode)
...
if mode == "🏠 Dashboard":
    ...
    col3.metric("Shadow Mode", "ON" if st.session_state.shadow_mode else "OFF")
...
# =========================
    ...
    description = st.text_area("Décrivez votre situation")

    uploaded_files = st.file_uploader("Documents", accept_multiple_files=True)

    if st.button("📩 Envoyer la demande"):

        client_id = create_client(nom, prenom, email, tel)
        dossier_id = create_dossier(client_id, description, "PLATEFORME_CLIENT")
        ...
        st.json(st.session_state.dossiers[dossier_id]["analysis"])

        st.warning("⚠️ Ceci est une assistance IA. Validation par un avocat requise.")

    st.markdown("---")
    st.error("🔴 URGENCE : Contacter le cabinet immédiatement")
...
# =========================
    ...
    st.subheader("👻 Shadow Mode: " + ("ON" if st.session_state.shadow_mode else "OFF"))

    for d_id, dossier in st.session_state.dossiers.items():

        client = st.session_state.clients.get(dossier["client_id"], {})

        with st.expander(f"📁 {client.get('nom','')} - {dossier['analysis']['urgence']}"):
            ...
            uploaded = st.file_uploader("Ajouter un document", key=d_id, accept_multiple_files=True)
            ...
            st.button("📞 Appeler client", key="call_" + d_id)

            st.info(f"📡 Source dossier : {dossier['source']}")
...
# =========================
    ...
    st.write("Assistant contextuel du cabinet")

    question = st.text_area("Pose une question sur un dossier")
    ...
    st.warning("⚠️ Outil d’assistance uniquement, validation avocat obligatoire")
...
# =========================
# NOUVEAU : MODE CLIENTS (CRM LIGHT)
# =========================
    ...
    for client_id, client in st.session_state.clients.items():
        ...
        with st.expander(f"👤 {client['nom']} {client['prenom']}"):
            ...
            for d_id in client.get("dossiers", []):
                ...
                if dossier:
                    if st.button(f"📂 Ouvrir dossier {dossier['analysis']['type']}", key="open_" + d_id):
                        st.session_state.selected_client_id = client_id
                        st.success("Dossier sélectionné (navigation future possible)")
