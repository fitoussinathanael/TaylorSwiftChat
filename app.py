        ...
        for r in rows:
            ...
            with st.expander(f"{r[2]} | {r[4]} | {r[5]}"):
                ...
                if role == "Client":
                    ...
                    st.write("## 👤 Espace client guidé")

                    st.info("Assistant IA : nous vous aidons à compléter votre dossier")

                    st.write("### 📌 Documents nécessaires")
                    st.write("- pièces d'identité")
                    st.write("- justificatifs fiscaux")
                    st.write("- documents comptables")

                    file = st.file_uploader("Uploader un document")

                    if file:
                        st.success("Document reçu ✔️")

                    st.write("### 🔄 Statut dossier")
                    st.write("Votre dossier est en cours de traitement par votre avocat")
                ...
                # -----------------------
                    ...
                    if st.button(f"Analyser dossier {dossier_id}", key=f"ai_{dossier_id}"):
                        ...
                        st.write("- incohérences possibles")
                        st.write("- risque de redressement partiel")

                        st.write("### 📌 Actions recommandées")
                        st.write("- relancer client pour pièces")
                        st.write("- vérifier cohérence fiscale")
