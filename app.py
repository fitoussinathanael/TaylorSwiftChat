...
# =========================================================
# PERFUSION ICU MONITOR (VERSION RÉA)
# =========================================================
DRUGS = {
...
def perfusion_monitor(drug, weight):
    d = DRUGS[drug]
    limits = ICU_LIMITS.get(drug, {"warn": 999, "crit": 999})

    st.subheader(f"💉 PERFUSION MONITOR ICU — {drug.upper()}")

    factor = st.slider(
        "Ajustement dose (x ICU)",
        0.5, 2.0,
        st.session_state.dose_factor,
        0.1
    )

    st.session_state.dose_factor = factor

    st.markdown("### 📊 Live perfusion")
    ...
    for dose in d["steps"]:
        ...
        with col1:
            st.write(f"**Dose {dose}**")

        with col2:
            st.write(f"Base: {base}")

        with col3:
            st.write(f"Réel: {real}")

        with col4:
            st.write(status)

        if real >= limits["crit"]:
            st.error("🚨 RISQUE VASOPRESSEUR ÉLEVÉ")
        elif real >= limits["warn"]:
            st.warning("⚠️ Zone de vigilance")

    st.info(f"📖 {drug} — surveillance continue requise")
