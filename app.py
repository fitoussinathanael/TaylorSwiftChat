...
st.sidebar.markdown("---")
st.sidebar.subheader("🔔 Alertes Cabinet")

urgent_count = 0

for dossier in st.session_state.dossiers.values():

    if dossier["analysis"]["urgence"] == "HIGH":
        urgent_count += 1

st.sidebar.write(f"🚨 Dossiers urgents : {urgent_count}")
st.sidebar.write(f"📁 Total dossiers : {len(st.session_state.dossiers)}")
...
# =========================
    ...
    for d_id, dossier in st.session_state.dossiers.items():
        ...
        with st.expander(
            ...
            # =========================
            # PRIORITÉ + IA CONFIDENCE
            # =========================

            st.write("### 🚨 Priorité dossier")
            st.warning(dossier["analysis"]["priority"])

            st.write(
                f"🧠 Confidence IA : {dossier['analysis']['confidence']}%"
            )
            ...
            if st.button(
                ...
                if new_task.strip():
                    ...
                    add_event(
                        d_id,
                        f"📝 Nouvelle action ajoutée : {new_task}"
                    )
            ...
            if st.button(
                "🧠 Générer email",
                key=f"generate_email_{d_id}"
            ):

                generated_email = f"""
Bonjour {client.get('prenom','')},
...
Merci de transmettre les éléments nécessaires
...
"""
                ...
                add_event(
                    d_id,
                    f"📨 Brouillon email généré : {email_type}"
                )

            if st.button(
                "📅 Programmer email",
                key=f"schedule_email_{d_id}"
            ):

                add_event(
                    d_id,
                    "📅 Email programmé par avocat"
                )

                st.success(
                    "Email programmé (simulation MVP)"
                )
