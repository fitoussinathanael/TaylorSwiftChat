...
if mode == "ICU FLOW":
    ...
    if user_input:
        ...
        if "propof" in text:
            risks.append("hypotension + sédation profonde")

        if "midaz" in text:
            risks.append("accumulation sédative")
        ...
        if "hypotension" in text:
            risks.append("déséquilibre hémodynamique")
        ...
        if "tension" in text or "norad" in text:
            ...
            detected_targets.append("hemodynamique")

        if "ventil" in text or "intub" in text:
            p["cibles"]["respiratoire"] = user_input
            detected_targets.append("respiratoire")

        if "sedation" in text or "glasgow" in text or "propof" in text:
            p["cibles"]["neuro"] = user_input
            detected_targets.append("neuro")

        if "diurese" in text or "creat" in text:
            p["cibles"]["renal"] = user_input
            detected_targets.append("renal")

        if "sepsis" in text or "infection" in text:
            p["cibles"]["infectieux"] = user_input
            detected_targets.append("infectieux")
        ...
        # -----------------------------
        answer = f"""
🏥 ICU FLOW V3 - {st.session_state.current_patient}
...
🧠 CIBLES ACTIVES : {', '.join(detected_targets) if detected_targets else 'aucune'}
...
👉 FLOW V3 multi-intelligent actif
"""
...
"""
            ...
            if found:
                d = drugs[found]
                ml_h = (d["dose"] * weight * 60) / d["conc"] if "µg" in d["unit"] else (d["dose"] * weight) / d["conc"]
