...
# =========================================================
# ICU FLOW STRUCTURÉ (CIBLES + GRAVITÉ)
# =========================================================
def flow(text):
    ...
    if any(x in t for x in ["dyspnée", "hypox", "oap", "pneumonie", "sdra"]):
        targets.append("🫁 Respiratoire")

    if any(x in t for x in ["choc", "hypotension", "lactate", "sepsis"]):
        targets.append("❤️ Hémodynamique")

    if any(x in t for x in ["coma", "agitation", "confusion"]):
        targets.append("🧠 Neurologique")

    if any(x in t for x in ["créatinine", "diurèse", "rein"]):
        targets.append("🧪 Rénal")

    out = "🧠 **ICU FLOW STRUCTURÉ**\n\n"

    out += "🎯 CIBLES\n"
    out += "\n".join([f"- {x}" for x in targets]) if targets else "- Non identifiées"

    out += "\n\n⚠️ GRAVITÉ\n"
    if "choc" in t or "noradrenaline" in t:
        out += "🔴 CRITIQUE"
    elif "dyspnée" in t:
        out += "🟠 SÉVÈRE"
    else:
    ...
    if rag:
        out += "\n\n📖 RAG\n"
        out += f"- {rag.get('classe','-')} / {rag.get('usage','-')}"
